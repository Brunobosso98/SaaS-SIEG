const XML = require('../models/xml.model');
...
exports.downloadXML = async (req, res) => {
  try {
    ...
    if (!cnpjId) {
      return res.status(400).json({ message: 'CNPJ ID is required' });
    }
    ...
    if (!user.siegKey) {
      return res.status(400).json({ message: 'SIEG key is required. Please add your SIEG key in settings.' });
    }
...
exports.getXMLHistory = async (req, res) => {
  try {
    const userId = req.user.id;
    const { cnpjId, documentType, startDate, endDate, status, page = 1, limit = 10 } = req.query;
...
// Get XML by ID
exports.getXMLById = async (req, res) => {
  try {
    const userId = req.user.id;
    ...
    if (!xml) {
      return res.status(404).json({ message: 'XML not found' });
    }
...
// Delete XML
exports.deleteXML = async (req, res) => {
  try {
    const userId = req.user.id;
    const xmlId = req.params.id;
...
exports.configureXMLDownloadSettings = async (req, res) => {
  try {
    const userId = req.user.id;
